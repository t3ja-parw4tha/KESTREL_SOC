...
export function TacticColumn({ tactic, techniques, onTechniqueClick }: TacticColumnProps) {
  ...
  return (
    <div className="rounded-xl border border-border bg-card/50 backdrop-blur-sm overflow-hidden flex flex-col">
      {/* Tactic header with gradient accent */}
      <div className="relative px-3 py-2.5 border-b border-border bg-gradient-to-r from-blue-500/10 via-transparent to-transparent">
        <div className="flex items-center justify-between gap-2">
          <h3 className="text-xs font-bold text-foreground uppercase tracking-wider leading-tight">
      ...
      <div className="p-2 space-y-1.5 flex-1">
        {techniques.length === 0 ? (
          <p className="text-[10px] text-muted-foreground text-center py-3 italic">No techniques</p>
        ) : (
          techniques.map((t) => (
            <TechniqueCard
              key={t.id}
